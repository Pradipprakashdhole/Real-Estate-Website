...
exports.addPropertyPage = (req, res) => {
  if (!req.session.user) return res.redirect("/login");// first you will login then you can add property
...
exports.addProperty = (req, res) => {
  upload(req, res, async function (err) {
    ...
    const { title, description, location, price, type } = req.body; // data uploding

    //  Correct the image path to include images/
    const imagePath = req.file ? `/images/${req.file.filename}` : "";// image uploding
    ...
    try {
      const property = new Property({
        ...
        category:type.toLowerCase(), //  normalize "Buy" → "buy"
...
/// GET: commercial Listings whit filter
exports.commercialProperties = async (req, res) => {
  const filters = { 
    type: "rent",         
    category: "commercial" 
  };
  ...
  try {
    const properties = await Property.find(filters);
    res.render("commercial", { properties });
  } catch (err) {
    res.send("Error fetching commercial properties: " + err);
  }
};
